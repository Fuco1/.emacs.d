...
* Nicer fontification
** DONE Fontify done checkbox items in org-mode                      :published:
...
* DONE Use org-radiobutton to select an option from a list           :published:
...
* DONE Multiline fontification with org-emphasis-alist               :published:
...
* Org protocol
** DONE Maximize the org-capture buffer                              :published:
...
* Logging
** DONE Use the input method from original org buffer in log note buffer :published:
...
* Goals, journaling and progress tracking with org-mode
:PROPERTIES:
...
* DONE Make agenda clockreport respect =org-extend-today-until=        :published:
...
Setup taken mostly from [[https://cestlaz.github.io/posts/using-emacs-26-gcal/][Using Emacs - 26 - Google Calendar, Org
Agenda]].  One exception is that I'm using a single-way sync, that is I
only fetch from google calendar and do not publish anything.

The reason is that I use multiple calendars (I basically have a google
account at every company I work for and a personal calendar) and the
...
(use-package org-gcal
  ...
  (setq org-gcal-client-id "781554523097-ocjovnfpqgtpoc4qv7ubr8c679t96bv7.apps.googleusercontent.com"
        org-gcal-client-secret "U5Bt-J2y7Vm69dEcRNkBLapE"
