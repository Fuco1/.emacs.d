...
** Make the whitespace before list items also fontified with =org-list-dt=

This will ensure that the levels are properly aligned when mixing
numbered lists and plain lists.

#+BEGIN_SRC elisp
(font-lock-add-keywords
 'org-mode
 `(("^\\([ \t]*\\)\\(?:[-+*]\\|[0-9]+[).]\\)[ \t]+" 1 'org-list-dt prepend))
 'append)
#+END_SRC
